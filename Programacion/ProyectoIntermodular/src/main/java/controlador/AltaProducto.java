...
public class AltaProducto extends HttpServlet {
    ...
    protected static Producto recogerProductoFormulario(HttpServletRequest request) {
        try {
            ...
            if (nombre != null && descripcion != null && precio != null && unidad != null && iva != null && stock != null && stockmin != null && !categorias.isEmpty()) {
                Usuario usuarioCrea = (Usuario) request.getSession().getAttribute("usuario");
                String rutaImagen = (fichero.getSize() != 0) ? ("img/productos/" + fileName) : null;
                //return new Producto(new ProductoDAO().siguienteCodigo(), utf8(nombre), utf8(descripcion), precio, utf8(unidad), iva, stock, stockmin, utf8(rutaImagen), usuarioCrea, LocalDateTime.now(), null, null, categorias);
                return new Producto(new ProductoDAO().siguienteCodigo(), iva, stock, stockmin, utf8(unidad), precio, utf8(nombre), utf8(descripcion), utf8(rutaImagen), LocalDateTime.now(), null, proveedor, usuarioCrea, null);
            }
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try ( PrintWriter out = response.getWriter()) {
            ...
            if (usuarioSesion == null || !usuarioSesion.getTipo().toString().equals("Administrador")/*isAdmin()*/) {
